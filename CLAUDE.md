...
- **`Conventions.md`** is the only document here that is not a note. It takes a plain
  name like every other file and sorts in among the zettels; the `_` prefix was dropped
...
deleted on 2026-08-20 because inventories belong there. The desktop project note went to
`~/projects/desktop` the same way. Do not recreate any of them here, and reference them
by path rather than wiki link: a link resolves only inside this graph.
