...
- [x] **Route the actionable items** (from the previous Someday list):
  ...
  - *All calendar events above scheduled ✓.*
- [x] Handle **Maeve builds** (crib, mobile arm, bookcase): they are multi-week projects, so the grouped project note `projects/Maeve builds.md` holds the build steps and the calendar holds only the shared target date (manual, external — scheduled ✓). *(step shells to fill.)*
