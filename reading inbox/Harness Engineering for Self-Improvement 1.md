...
An MCE skill $s \in \mathcal{S}$ defines a context function $c_{s} = \left(\rho_{s} , F_{s}\right)$ and maps an input $x$ to context $c = F_{s} \left(x ; \rho_{s}\right)$, where:

- $\rho_{s} = \left\{\rho_{1} , \ldots , \rho_{m}\right\}$ are static components (prompts, knowledge bases, code libraries).
- $F_{s} = \left\{F_{1} , \ldots , F_{k}\right\}$ are dynamic operators (search, selection, filtering, formatting).
...
The skill database tracks the history of previous skills, context functions and eval metrics $\mathcal{H}_{k - 1} = \left\{\right. \left(s_{i} , c_{i} , J_{i}^{\text{train}} , J_{i}^{\text{val}}\right) \left.\right\}_{i = 1}^{k - 1}$. A meta-level agent performs agentic [crossover](https://en.wikipedia.org/wiki/Crossover_\(evolutionary_algorithm\)) over prior skills to create a new skill given a task $\tau$: $s_{k} = \text{crossover} \left(\tau , \mathcal{H}_{k - 1}\right)$.

Then a base-level context engineer executes the skill $s_{k}$ and learns the context function from rollout feedback $\mathcal{R}_{k}$, guided by the current skill: $c_{k} = \text{engineer} \left(\tau , s_{k} ; c_{k - 1}^{*} , \mathcal{R}_{k}\right)$.
