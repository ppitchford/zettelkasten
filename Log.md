...
The running record and capture surface — the one place to jot, newest day on top. Under today's `## YYYY-MM-DD`, drop whatever lands: a task, a fleeting thought, what happened, a note you made. It is not a task ledger — the captures get sorted out on review: tasks to Things3 or the calendar, an idea worth developing into a note, the rest kept as record or deleted.
...
- 11:02 — Moved frame, ArtSpacesSoCal, and Personal Website's constraints out of the vault and into their own repos. Project detail belongs beside the code it describes, and the vault keeps only what would survive the repo being deleted.
- 14:58 — Wrote three zettels on what the plain vanilla web trades away, and corrected the IWE inclusion-link rule after testing it: a link alone as its own paragraph is an inclusion and pulls the target's whole body in, so the distinction is the paragraph and not the line. Set a markdown wrap width at the same time.
- 16:34 — Flattened the vault. `projects/` and `sources/` held three files between them, and under IWE a folder path is the document key, so folders address rather than organize. Rebuilt the projects ladder in the style guide as part of the move.
- 17:08 — Moved the daily and weekly review routines into Things3, then deleted `_Projects.md` and `_Learning.md`. A list of open commitments is an inventory, and Things3 already holds every item on it.
- 17:43 — Normalized the log's markup and wrote its voice rule into the style guide, then renamed the guide to `Conventions.md` and dropped the underscore prefix, which had nothing left to sort above.
...
- 11:29 — Replaced the Rosé Pine theme bundles with Iceberg and pointed hyprlock's background at the current wallpaper.
...
- 12:57 — Recorded the dwl migration in the Claude Code guidance files and tightened the instructions around asserting system state without verifying it. Replaced the old `launcher` script with native fuzzel for application launching, and repurposed it as `varia`, a dispatcher covering the six modes fuzzel cannot handle: calc, files, emoji, symbols, clipboard, and system. Fixed the fuzzel theme by moving `border-width` and `border-radius` into the `[border]` section where they are actually read, switched the icon theme to Papirus for its full 88,300-icon set, and raised the file-search line count. Regenerated the manifests after installing Papirus and removing GIMP and foot.
...
- 13:40 — Cut over to dwl as the login compositor and removed Quickshell and MangoWM from the boot path. Configuration was now compile-time, so every change cost a rebuild and a session.
...
- 15:16 — Paid the California Franchise Tax Board penalty.
- 16:54 — Installed `xdg-desktop-portal-gtk`, after which Helium and Obsidian followed the theme live through ornatus. `sweep_unattached`'s warning in `/tmp/ornatus.log` was still the only instrument for the unproven startup race fix.
- 07:17 — Repaired the `launcher` system menu — four of five actions were silently broken (swaylock, `loginctl`, `mmsg`); replaced with hyprlock and runit commands.
...
## 2026-08-10
- 10:00 — Attended the anatomy scan appointment with Rachel.
- 11:30 — Had brunch with Rachel.
- 12:30 — Removed the shower doors in the second bathroom.
- 13:30 — Updated my PMI community profile.
- 18:00 — Interviewed with a PMI-SD representative for the mentorship program.
...
## 2026-08-07
- Filed 25 articles into the reading inbox.

## 2026-08-06
- 13:10 — Submitted an application for the Director of Product role at Flux Power, which Michele, an employee there, recommended I apply for.

## 2026-07-31
- 12:03 — Applied to be a mentor for the PMI San Diego chapter this fall.

## 2026-07-28
- 09:30 — Time-stamped yesterday's entries from the git history, so each bullet carries the time its thread of work began. Settled that work is recorded under the date its session began, which keeps a sitting that runs past midnight with the evening it started and moved the Morning Review rewrite back under 07-27; decided against recording a time frame for each entry, since a range claims a continuity the work rarely has and forces a rule for splitting a session at midnight. Switched to 24-hour times to match the `YYYY-MM-DD` dates and the `YYYYMMDDHHmmss` id, and settled the entry format as a bullet, the time, an em dash, then the sentence.
- 13:00 — Began the drive from Park City to San Marcos.

## 2026-07-27
- 12:57 — Overhauled the vault: one running log replacing the weekly-note stream, every zettel renamed to its title, and the system notes pinned above them. Formalized the task system around three surfaces — the log, project notes, and the calendar — run by a daily Morning Review.
- 13:39 — Fixed the Obsidian Web Clipper, which had been wrapping author names in dead wikilinks, and settled how clipped articles flow into the reading inbox.
- 18:22 — Adopted project-based learning, each skill learned by building a real project and the learning kept in that project's note, with `_Learning` as the counterweight for the deliberate study that building alone skips.
- 20:31 — Scoped ArtSpacesSoCal as an active learn-by-building Go project: Claude tutors rather than writes the code, and a Go refresher comes first.
- 21:03 — Set up a writing folder for my original writing and moved my first two essays into it.
- 21:18 — Rewrote the Morning Review as five steps against Tiago Forte's model, extending the mnemonic to Every Commitment Needs Tracking Today and giving the choice of what to do today its own step. Gave both reviews their own headings so the same text serves as the description of the recurring calendar events, and standardized on "an idea worth developing" over "a thought with legs".
- 00:38 — Settled that a phrase or idea extracted from a book becomes a zettel directly, because the reading inbox holds only clipped articles that have not been judged yet.
- Cleared the four items routed from the last weekly plan: the hotel for the UT–CA drive, Shane Peters forwarding his City of San Marcos emails, the Things3 audit and migration into Obsidian, and dated and recurring tasks into Google Calendar.

## 2026-04-23
- 08:30 — Appointment with Dr. Hirsch at *San Diego Ear, Nose, and Throat*. My sleep test score was 5, which is borderline for sleep apnea. The medication has helped my breathing, but I have a definitive deviated septum on the right side. The recommendation was to lose 10 to 20 lbs, sleep on my side, use nasal strips, and keep addressing my allergies, then ask about a septoplasty if the trouble continues over the next three to six months.
- Distilled Sari Azout's [Letter to a friend who is thinking of starting something new](https://www.sariazout.com/p/letter-to-a-friend) into six zettels and a seventh that synthesizes them: [[Insecurity work produces no progress but repeats without notice]], [[Insecurity work is dangerous because it is frictionless]], [[Ideas require emotional capital as much as financial capital]], [[The passion economy places a double burden on work]], [[Pre-commit your stopping conditions before the pressure arrives]], [[Industry norms pull a business toward conformity]], and [[The antidote to outcome-driven anxiety is treating the process as the destination]].
- Downloaded Obsidian and migrated the note system, which took very little effort since the Neovim setup was already Markdown.

## 2026-04-22
- Wrote three zettels: [[Solving the right problem unlocks creative immersion]], [[The moment to begin writing is a threshold, not a schedule]], and [[Emotional restraint is most powerful at the moment it breaks]].
...
- Wrote four zettels: [[A novel begins with a vital, surprising, unanswered question]], [[A novel requires months or years of incubation before serious writing begins]], [[The first challenge in writing prose is finding the voice and tone]], and [[Overcoming the confidence struggle requires giving yourself permission to write privately]].

## 2026-04-02
- Completed the overhaul of my LinkedIn profile, working through the experience, education, and skills sections.
- Made more progress on my Apple to Linux migration, setting up screenshot utilities and 1Password.
- Wrote five zettels from the article on surface acting: [[Surface acting]], [[Social expectations drive performative emotions]], [[Genuine emotions build psychological resources]], [[Anhedonic flatness]], and [[Contentment performance creates a vicious cycle]].

## 2026-04-01
- Purchased my first orbital sander.
- Worked with Claude to define the target industries and roles for my job search.
- Spent more time than I should have working through my resume.

## 2026-03-31
- Spent most of the day migrating from Things3 to Todoist. I worked with Claude to define a new way of managing tasks: projects stay at a surface level, the daily review carries most of the weight, and the weekly review audits work in progress.
- Worked out with Claude that I can position myself as a data or technical product manager, and that the personal website is a component of the job search.
- Paid the annual CAM difference for the studio space.

## 2026-03-29
- Read several more chapters of *Malleus*, the second Eisenhorn book.
- Set up my zettelkasten, opting for the `zk` plugin rather than cobbling together two or three others.
- Added shell scripts to my `.zshrc` in case I ran into trouble with `zk` and needed to deactivate it.
- Migrated two notes from my git repository into the new local directory.

## 2026-03-26
- Created a git repository for the notes on GitHub.
- Implemented a spell-check plugin for Neovim.
- Implemented a feature that saves zettels automatically, in pure Neovim Lua with built-in commands.

## 2026-03-25
- Completed the "Premium Audit" requested by Employers.
- Switched my shell on the Framework 13 from bash to zsh. I ported the `.zshrc` from my MacBook Air and stripped out the references to macOS-only software.
- Set up this note-taking system on the Zettelkasten method, Luhmann's slip-box.
- Added scripts to `.zshrc` that generate each note type from a template.
- Wrote [[A zettel is atomic and self-contained]] and [[A zettel gains meaning through its connections]].
