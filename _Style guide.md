...
The system is run by a daily **Morning Review**: skim the calendar for what's ahead; process Gmail, routing each item to a task, a clip, or an event; sort the top of the log; confirm each active project has a next action; then, under today's heading, set what must happen today and send dated items to the calendar.
