...
- **Log** (`_Log.md`) is the one place to jot — daily record and capture in one file, newest day on top. Under each `## YYYY-MM-DD`, drop tasks, fleeting thoughts, what happened, and links to notes made that day (a dated provenance trail). It is *not* a task ledger: in the Morning Review its captures are sorted out — tasks to Things3 or the calendar, an idea worth developing to a `#seedling` note, the rest kept as record or deleted.
...
4. **Tasks.** State each task you collected as a concrete next action. Add it to Things3, or to the calendar if it's time-bound. Confirm every active project has a next action there, and scan `#waiting` for anything stalled.
